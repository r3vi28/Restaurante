...
using Respuesta = Result<PedidoStatusRecord>;
using Request = PedidoStatusRouteManager;
...
public class GetById : EndpointBaseAsync.WithRequest<Request>.WithActionResult<Respuesta>
{
    ...
    [HttpGet(PedidoStatusRouteManager.GetById)]
    public override async Task<ActionResult<Respuesta>> HandleAsync([FromRoute] Request request, CancellationToken cancellationToken = default)
    {
        try
        {
            var pedido = await dbContext.PedidoStatuses
            .Where(p => p.Id == request.Id)
            .Select(pedido => pedido.ToRecord())
            .FirstOrDefaultAsync(cancellationToken);

            if(pedido == null)
            return Respuesta.Fail($"No fue posible encontrar el estado de pedido '{request.Id}'");
